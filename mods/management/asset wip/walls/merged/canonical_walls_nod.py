...
MEG_RA = "/Users/moritzgiuliani/Library/Application Support/Steam/steamapps/common/CnCRemastered/Data/TEXTURES_RA_SRGB.MEG"
contents, raw = m.parse_meg(MEG)
contents_ra, raw_ra = m.parse_meg(MEG_RA)
_zips = {}
TILE = 128
...
def get_zip(zn):
    if zn not in _zips:
        keys_td = [k for k in contents if k.upper().endswith(zn)]
        if keys_td:
            off, size = contents[keys_td[0]]
            _zips[zn] = zipfile.ZipFile(io.BytesIO(raw[off:off + size]))
        else:
            key = [k for k in contents_ra if k.upper().endswith(zn)][0]
            off, size = contents_ra[key]
            _zips[zn] = zipfile.ZipFile(io.BytesIO(raw_ra[off:off + size]))
    return _zips[zn]
...
def sbag(i): return _try_load("FENC.ZIP", "fenc", i + SBAG_OFF, i)
...
if __name__ == "__main__":
    sheet(True, "V3: Brick + Sandbag + Cyclone", "nod_sheet_with_cycl.png")
    sheet(False, "V3: Brick + Sandbag", "nod_sheet_without_cycl.png")
    assembly({(0, 0): S, (0, 1): N | S, (0, 2): N | E, (1, 2): E | W, (2, 2): W}, 3, 3, "nod_L_wall.png")
    assembly({(1, 0): S, (1, 1): N | E | S | W, (1, 2): N, (0, 1): E, (2, 1): W}, 3, 3, "nod_cross.png")

    BITS_DIR = "/Users/moritzgiuliani/Documents/openRA Projekte/TiberianDawnHD/mods/cnc/bits"
    bake(False, f"{BITS_DIR}/aot-wall-nod.png")
    bake(True, f"{BITS_DIR}/aot-wall-nod-DUMMY.png")
    print("done")
