...
namespace OpenRA.Network
{
	...
	{
		// aotmod: increased from 7. Save-load desync debugging needs the buffer to still contain the
		// saved sync frame both when the save-time reference is dumped and when the load detects the
		// desync (which can be several frames after the sync frame). 256 frames (~10s) is ample and
		// only retained when sync reports are actually being generated (debug / save-load).
		const int NumSyncReports = 256;
		...
		internal void DumpSyncReport(int frame)
		{
			...
			foreach (var r in syncReports)
			{
				...
				if (r.Frame == frame)
				{
					...
					WriteReportHeaderTo("sync");
					if (Game.IsHost)
						Log.Write("sync", "Player is host.");
					WriteReportBodyTo("sync", r);
				}
			}
		...
		// aotmod: dump the full buffered per-trait sync state to a "savesync" file when a game save is
		// written. This is the reference to diff against the "syncreport" produced if the save later
		// desyncs on load: the trait whose NamesValues differ at the saved sync frame is the culprit.
		// Requires sync reports to be generated during the play session (Settings.Debug.EnableSyncReports).
		internal void DumpSaveReferenceReport(int saveFrame)
		{
			var timestamp = DateTime.UtcNow.ToString("yyyy-MM-ddTHHmmssZ", CultureInfo.InvariantCulture);
			var reportName = $"savesync-{timestamp}-{orderManager.LocalClient?.Index}.log";
			Log.AddChannel("sync-save", reportName);

			var recordedFrames = new List<int>();
			var wrote = false;
			WriteReportHeaderTo("sync-save");
			foreach (var r in syncReports.OrderBy(x => x.Frame))
			{
				if (r.Frame == 0)
					continue;

				recordedFrames.Add(r.Frame);
				WriteReportBodyTo("sync-save", r);
				wrote = true;
			}

			Log.Write("sync-save", "Save Reference System Info:");
			Log.Write("sync-save", $"Save sync frame (approx): {saveFrame}");
			Log.Write("sync-save", "Recorded frames: " + string.Join(",", recordedFrames));
			if (!wrote)
				Log.Write("sync-save", "No sync reports buffered. Enable Settings.Debug.EnableSyncReports before playing to capture a reference.");
		}

		void WriteReportHeaderTo(string channel)
		{
		...
		void WriteReportBodyTo(string channel, Report r)
		{
