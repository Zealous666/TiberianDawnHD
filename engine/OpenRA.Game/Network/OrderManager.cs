...
namespace OpenRA.Network
{
	public sealed class OrderManager : IDisposable
	{
		...
		// aotmod: write a per-trait sync reference file when a save is made, to diff against the
		// syncreport produced if that save later desyncs on load. See SyncReport.DumpSaveReferenceReport.
		public void DumpSaveReferenceReport()
		{
			syncReport.DumpSaveReferenceReport(NetFrameNumber);
		}
		...
		public void StartGame()
		{
			...
			// aotmod: ALSO generate them when loading a save game. A save load replays via a
			// ReplayConnection (which normally suppresses reports) but DOES run a real sync check at
			// the saved frame (GameSaveLastSyncFrame is set from the save metadata order before this
			// runs). Without this the report buffer stays empty ("Recorded frames: 0,...") and a
			// save-load desync yields "No sync report available". Forced on there regardless of the
			// EnableSyncReports debug setting, since diagnosing that desync is the whole point and the
			// cost only applies to the one-off load replay.
			var isLoadingSaveGame = GameSaveLastSyncFrame >= 0;
			generateSyncReport = isLoadingSaveGame
				|| (Connection is not ReplayConnection && LobbyInfo.GlobalSettings.EnableSyncReports);

			// aotmod: per-frame sync trace (see ProcessOrders). A distinct file per session so a play
			// session and its later save-load replay produce two files to diff for the first divergence.
			// "-load" suffix marks the replay side so the two are easy to tell apart.
			if (generateSyncReport)
			{
				var stamp = DateTime.UtcNow.ToString("yyyy-MM-ddTHHmmssZ", System.Globalization.CultureInfo.InvariantCulture);
				var suffix = isLoadingSaveGame ? "-load" : "-play";
				Log.AddChannel("synctrace", $"synctrace-{stamp}{suffix}.log");
			}
		...
		void ProcessOrders()
		{
			...
			// aotmod: compute the sync hash once so it can be sent AND traced. During a save-load
			// replay the built-in check only compares the single saved frame, so it cannot localise
			// an EARLY divergence (e.g. a save at Age 2 that reloads looking like Age 0). The per-frame
			// trace below lets us diff a play session against its save-load replay and find the FIRST
			// frame whose hash differs -- that frame's cause is the root desync.
			...
			if (generateSyncReport)
				Log.Write("synctrace", $"{NetFrameNumber}\t{syncHash}\t{World.SharedRandom.TotalCount}\t{World.SharedRandom.Last}");
