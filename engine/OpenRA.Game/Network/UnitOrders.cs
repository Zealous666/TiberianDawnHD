		...
		internal static void ProcessOrder(OrderManager orderManager, World world, int clientId, Order order)
		{
			switch (order.OrderString)
			{
				...
				case "GameSaved":
					...
					// aotmod: on a REAL save (not while replaying a save-load), dump the per-trait sync
					// reference so a later save-load desync can be pinpointed by diffing it against the
					// load's syncreport at the saved frame.
					if (world != null && !world.IsLoadingGameSave)
						orderManager.DumpSaveReferenceReport();
					break;
