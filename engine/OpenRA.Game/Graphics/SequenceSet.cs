...
namespace OpenRA.Graphics
{
	...
	public sealed class SequenceSet : IDisposable
	{
		...
		{
			// aotmod: guard so a cached/reused SequenceSet is not loaded twice. ResolveSprites
			// consumes (removes) each token from the SpriteCache, so a second call would throw.
			if (spritesLoaded)
				return;
			spritesLoaded = true;
