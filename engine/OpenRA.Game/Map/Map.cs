...
namespace OpenRA
{
	...
	public sealed class Map : IReadOnlyFileSystem, IDisposable
	{
		...
		// aotmod: true when Sequences is the shared, ModData-owned atlas (do not dispose it here).
		bool sequencesFromCache;
		...
		void PostInit()
		{
			...
			// aotmod: reuse the sprite atlas across worlds when the tileset matches (see
			// ModData.GetOrCreateSequences). A cached/shared set must not be disposed by this map.
			(Sequences, sequencesFromCache) = modData.GetOrCreateSequences(this, Tileset, SequenceDefinitions);
		...
		public void Dispose()
		{
			// aotmod: only dispose a private (map-owned) SequenceSet. The shared cached atlas is
			// owned by ModData and reused by later worlds, so disposing it here would break reuse
			// and free GPU sheets still referenced by the next map.
			if (!sequencesFromCache)
				Sequences.Dispose();
		}
	}
}
