...
namespace OpenRA
{
	...
	public sealed class ModData : IDisposable
	{
		...
		public void PrepareMap(Map map)
		{
			...
			// aotmod: builds the sprite atlas on a cache miss (multi-second) or is a cheap no-op
			// when this world reuses a cached same-tileset atlas (see GetOrCreateSequences).
			using (new Support.PerfTimer("LoadSprites"))
		...
		/// skirmish skips the multi-second sprite rebuild (the actor sprites dominate and are
		/// tileset-independent; a different tileset is a genuine rebuild). Maps that inject their
		/// own sequence YAML always get a private, non-cached set. Only ONE set is kept alive
		/// (memory budget): a tileset change disposes the previous set before building the new one,
		/// so peak memory is unchanged from the previous build-every-load behaviour.
		/// The cached set is built against DefaultFileSystem (stable mod packages) so it does not
		/// depend on any transient map's lifetime.
		/// </summary>
		public (SequenceSet Sequences, bool FromCache) GetOrCreateSequences(
			IReadOnlyFileSystem mapFileSystem, string tileSet, MiniYaml additionalSequences)
		{
			if (additionalSequences != null)
				return (new SequenceSet(mapFileSystem, this, tileSet, additionalSequences), false);

			if (cachedSequences != null && cachedSequencesKey == tileSet)
			{
				Log.Write("perf", $"SequenceCache: reusing atlas for tileset {tileSet}");
				return (cachedSequences, true);
			}

			Log.Write("perf", $"SequenceCache: building atlas for tileset {tileSet}" +
				(cachedSequences != null ? $" (evicting {cachedSequencesKey})" : ""));
			cachedSequences?.Dispose();
			cachedSequences = new SequenceSet(DefaultFileSystem, this, tileSet, null);
			cachedSequencesKey = tileSet;
			return (cachedSequences, true);
		}
		...
		public void Dispose()
		{
			// aotmod: dispose the reusable sprite atlas (see GetOrCreateSequences). Maps that use it
			// no longer own it, so it must be released here when the mod is torn down.
			cachedSequences?.Dispose();
