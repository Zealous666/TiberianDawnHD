...
namespace OpenRA
{
	...
	public sealed class Sound : IDisposable
	{
			...
			bool relative, WPos pos, float volumeModifier, bool attenuateVolume)
		{
			...
			{
				// A MISSING SOUND MUST NEVER END A GAME (AoT 2026-08-05). This threw, and a hard throw
				// from PlayNotification is a crash to desktop: two full test sessions were lost to
				// exactly that -- clicking a charged Age upgrade without the credits for it reaches
				// AutoActivateSpawnActorPower's hardcoded "InsufficientFunds", and any spy infiltration
				// reaches "BuildingInfiltrated". check-yaml catches undefined references declared in
				// YAML, but it cannot see a notification name that only exists as a C# string literal,
				// so those two got all the way to a player mid-match.
				//
				// Logged once per name rather than silently swallowed: a missing notification is still
				// a mod bug worth fixing, it is simply not worth killing the session over.
				if (!rules.NotificationsPools.Value.TryGetValue(definition, out var p))
				{
					if (reportedMissingNotifications.Add(definition))
						Log.Write("sound", $"Missing notification `{definition}` in pool `{type}` -- ignored (see Sound.cs)");
		...
		readonly HashSet<string> reportedMissingNotifications = [];
