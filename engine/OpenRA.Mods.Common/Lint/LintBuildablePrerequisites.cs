				...
				try
				{
					...
					if (bi != null)
						foreach (var prereq in bi.Prerequisites)
							// aotmod: "producer:"-Prereqs werden gegen die Conditions des produzierenden
							// Gebaeudes geprueft (ProductionQueue), nicht gegen den Spieler-Tech-Tree ->
							// sie werden von keinem ProvidesPrerequisite bereitgestellt und sind hier ok.
							if (!prereq.StartsWith("~disabled", StringComparison.Ordinal)
								&& !prereq.StartsWith("producer:", StringComparison.Ordinal)
								&& !providedPrereqs.Contains(prereq.Replace("!", "").Replace("~", "")))
