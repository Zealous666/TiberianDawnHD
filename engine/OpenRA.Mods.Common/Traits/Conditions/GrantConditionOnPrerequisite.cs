...
namespace OpenRA.Mods.Common.Traits
{
	...
	// Registration is tied to Created/Disposing rather than AddedToWorld/RemovedFromWorld
	// (aot patch). A passenger boarding Cargo (garrison structures, transports, etc.) gets
	// RemovedFromWorld called on it, which used to unregister it from the tech tree here -
	// any prerequisite researched while it sat boarded was silently missed, and only synced
	// back once (if ever) the passenger left the carrier again. Prerequisite availability is
	// player-level tech state, not something that should pause just because the actor is
	// temporarily out of world.
	public class GrantConditionOnPrerequisite : INotifyCreated, INotifyActorDisposing, INotifyOwnerChanged
	{
