...
{
	public class MinelayerInfo : ConditionalTraitInfo
	{
		[ActorReference]
		...
		[Desc("Condition granted by AmmoPool when ammo > 0. Deploy button is disabled when this condition is absent.")]
	...
	public class Minelayer : ConditionalTrait<MinelayerInfo>, INotifyCreated, IIssueOrder, IResolveOrder, ISync, IIssueDeployOrder, IOrderVoice, ITick
	{
		...
		public override IEnumerable<VariableObserver> GetVariableObservers()
		{
			...
			if (!string.IsNullOrEmpty(Info.AmmoCondition))
				yield return new VariableObserver(AmmoConditionChanged, new[] { Info.AmmoCondition });
		}

		void AmmoConditionChanged(Actor self, IReadOnlyDictionary<string, int> conditions)
		{
			hasAmmo = conditions.GetValueOrDefault(Info.AmmoCondition) > 0;
		}
		...
		IEnumerable<IOrderTargeter> IIssueOrder.Orders
		{
			get
			{
				...
				if (!string.IsNullOrEmpty(Info.AmmoCondition) && !hasAmmo)
					yield break;
		...
		bool IIssueDeployOrder.IsDeployVisible(Actor self) => !IsTraitDisabled && (string.IsNullOrEmpty(Info.AmmoCondition) || hasAmmo);
		...
		bool IIssueDeployOrder.CanIssueDeployOrder(Actor self, bool queued)
		{
			...
			// Otherwise fall back to pool lookup.
			if (!string.IsNullOrEmpty(Info.AmmoCondition))
			{
				if (!hasAmmo)
					return false;
