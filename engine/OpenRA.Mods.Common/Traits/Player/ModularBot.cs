...
namespace OpenRA.Mods.Common.Traits
{
	...
	public sealed class ModularBot : ITick, IBot, INotifyDamage
	{
		...
		void ITick.Tick(Actor self)
		{
			...
			{
				// aotmod: when the bot-module sync check is enabled, verify EACH module individually and
				// LOG the offender (with its SharedRandom draw count delta) instead of throwing an opaque
				// exception from the whole loop. A bot that changes synced state -- e.g. draws SharedRandom
				// via some engine helper -- is invisible during a save-load replay (bots are suppressed
				// while IsLoadingGameSave), so that play-only draw is exactly what desyncs the reload.
				// This pinpoints the module in a short live game, with no save/load needed.
				if (Game.Settings.Debug.SyncCheckBotModuleCode)
				{
					...
					foreach (var t in tickModules)
					{
						...
						var syncBefore = world.SyncHash();
						var rngBefore = world.SharedRandom.TotalCount;
						var cashBefore = pr != null ? pr.Cash + pr.Resources : 0;

						t.BotTick(this);

						// Two independent leakage channels, both invisible on a save-load replay (bots are
						// suppressed while IsLoadingGameSave): a change to the synced hash (e.g. a SharedRandom
						// draw), and a change to real cash/resources (TakeCash/GiveCash) which is NOT part of
						// the order stream. Either one desyncs the reload. Log both, do not crash.
						if (world.SyncHash() != syncBefore)
							Log.Write("debug", $"[BotSyncViolation] {t.GetType().Name} changed synced state " +
								$"(SharedRandom +{world.SharedRandom.TotalCount - rngBefore}) player={player.InternalName} tick={world.WorldTick}");

						if (pr != null && pr.Cash + pr.Resources != cashBefore)
							Log.Write("debug", $"[BotCashLeak] {t.GetType().Name} moved cash directly " +
								$"({cashBefore} -> {pr.Cash + pr.Resources}) player={player.InternalName} tick={world.WorldTick}");
					}
				}
				else
				{
