...
namespace OpenRA.Mods.Common.Traits
{
	...
	public class ProductionQueueInfo : TraitInfo, IRulesetLoaded
	{
		...
		[Desc("If true, all registered items are always visible in the production palette, even if their prerequisites are not met. " +
			"Items that are neither visible nor buildable will use the widget's LockedIconCollection sprite.")]
		public readonly bool ShowLockedItems = false;

		[Desc("If true, the production palette draws the widget's UpgradedText overlay over items " +
			"whose BuildLimit has been reached (for one-time purchases in upgrade queues).")]
		public readonly bool ShowUpgradedOverlay = false;
	...
	public class ProductionQueue : IResolveOrder, ITick, ITechTreeElement, INotifyOwnerChanged, INotifyKilled, INotifySold, ISync, INotifyTransform, INotifyCreated
	{
		...
		public virtual IEnumerable<ActorInfo> AllItems()
		{
			...
			if (Info.ShowLockedItems)
			{
				// One palette slot per LockedGroup: if any group member is visible it claims
				// the slot with its real icon; a fully hidden group shows a single placeholder.
				...
				foreach (var kv in Producible)
				{
					...
					else if (bi.ShowWhenLocked && !kv.Value.WasEverVisible)
					{
						// Placeholder candidate; the group is represented by its
						// earliest palette slot for a stable position.
						if (!lockedGroups.TryGetValue(group, out var existing) ||
							bi.BuildPaletteOrder < existing.TraitInfo<BuildableInfo>().BuildPaletteOrder)
							lockedGroups[group] = kv.Key;
					}
				}

				foreach (var kv in lockedGroups)
					if (!visibleGroups.Contains(kv.Key))
						result.Add(kv.Value);
		...
		public bool IsLocked(ActorInfo actor)
		{
			if (!Producible.TryGetValue(actor, out var ps))
				return true;
			return !ps.Buildable && !ps.Visible;
		}

		// Names of producible items whose BuildLimit is already reached by owned actors.
		// Used by the palette to draw the "Upgraded" / "Maximum Built" overlays.
		// Owned actors are counted per LockedGroup (fallback: actor name) so age-variant
		// chains keep the overlay when the palette proxy switches to a later variant.
		public HashSet<string> ItemsAtBuildLimit()
		{
			...
			foreach (var kv in Producible)
			{
				var bi = kv.Key.TraitInfo<BuildableInfo>();
				if (bi.BuildLimit > 0)
					(limits ??= [])[bi.LockedGroup ?? kv.Key.Name] = bi.BuildLimit;
			}

			// Skip the world scan entirely when nothing in this queue is limited.
			if (limits == null)
				return result;

			var counts = new Dictionary<string, int>();
			foreach (var a in Actor.Owner.World.ActorsHavingTrait<Buildable>())
			{
				if (a.Owner != Actor.Owner)
					continue;

				var bi = a.Info.TraitInfo<BuildableInfo>();
				var group = bi.LockedGroup ?? a.Info.Name;
			...
			{
				var bi = kv.Key.TraitInfo<BuildableInfo>();
				if (bi.BuildLimit > 0 && counts.GetValueOrDefault(bi.LockedGroup ?? kv.Key.Name) >= bi.BuildLimit)
	...
	public class ProductionState
	{
		...
		public bool WasEverVisible = false;
	}
