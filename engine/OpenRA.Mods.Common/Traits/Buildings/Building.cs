...
namespace OpenRA.Mods.Common.Traits
{
	...
	public class BuildingInfo : TraitInfo, IOccupySpaceInfo, IPlaceBuildingDecorationInfo
	{
		...
		{
			var requiresBuildableAreas = ai.TraitInfos<RequiresBuildableAreaInfo>();
			var mapBuildRadius = world.WorldActor.TraitOrDefault<MapBuildRadius>();

			if (!requiresBuildableAreas.Any() || p.PlayerActor.Trait<DeveloperMode>().BuildAnywhere)
				return true;
			...
			// aotmod: OR-logic across multiple RequiresBuildableArea instances (e.g. standard
			// building-type at Adjacent:2 OR outpost-type at Adjacent:6). Satisfied when ANY
			// instance finds a matching GivesBuildableArea source close enough.
			foreach (var requiresBuildableArea in requiresBuildableAreas)
			{
				...
				if (nearnessCandidates.Any(a => buildingTiles.Any(b =>
