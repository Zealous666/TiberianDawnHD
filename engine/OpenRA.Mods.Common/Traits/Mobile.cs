...
namespace OpenRA.Mods.Common.Traits
{
		...
		IFacing, IDeathActorInitModifier, INotifyAddedToWorld, INotifyRemovedFromWorld, INotifyBlockingMove, IActorPreviewInitModifier, INotifyBecomingIdle, ISync
	{
		...
		IPreventsIdleLayerReturn[] preventsIdleLayerReturn;
		...
		protected override void Created(Actor self)
		{
			...
			// aotmod (2026-07-29): see OnBecomingIdle - traits that keep the actor on its custom layer.
			preventsIdleLayerReturn = self.TraitsImplementing<IPreventsIdleLayerReturn>().ToArray();
		...
		void INotifyBecomingIdle.OnBecomingIdle(Actor self)
		{
			...
			// aotmod (2026-07-29): a trait may veto the automatic return to the ground layer, so an
			// ambushing subterranean unit can stay buried while idle instead of popping up at once.
			foreach (var p in preventsIdleLayerReturn)
				if (p.PreventsIdleLayerReturn(self))
					return;
