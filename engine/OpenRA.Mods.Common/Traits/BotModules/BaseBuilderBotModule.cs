...
namespace OpenRA.Mods.Common.Traits
{
	...
	public class BaseBuilderBotModuleInfo : ConditionalTraitInfo, NotBefore<ResourceMapBotModuleInfo>, NotBefore<IResourceLayerInfo>
	{
		...
		[Desc("Ensure at least this many SiloTypes buildings early (prioritised right after power, before refineries), ",
			"each placed as close as possible to the nearest ore mine (an actor with the OreMineDurability trait). ",
			"Intended for economies where a silo is the delivery dock for ore transporters. 0 disables this behaviour.")]
		public readonly int SilosNearOreMines = 0;
