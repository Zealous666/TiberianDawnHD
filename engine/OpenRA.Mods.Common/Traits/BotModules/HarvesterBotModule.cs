		...
		void IBotTick.BotTick(IBot bot)
		{
			...
			if (--scanForIdleHarvestersTicks <= 0)
			{
				...
				if (unitBuilder != null && Info.HarvesterTypes.Count > 0)
				{
					...
					if (harvCountTooLow)
					{
						// aotmod fix (User 2026-08-02: "Gegner fordert ploetzlich ganz viele Harvester an,
						// er sollte strikt nur Ersatz anfordern"). Two separate holes let this scan pile up
						// harvesters without bound:
						//   1. The already-requested guard looked at the RANDOMLY PICKED type only. With
						//      more than one entry in HarvesterTypes (this mod ships aot-harv +
						//      aot-harv-nod) a pending request for one type is invisible when the dice land
						//      on the other.
						//   2. Far worse: UnitBuilderBotModule removes a request from queuedBuildRequests
						//      on the very next tick after handing it to BuildUnit -- so the guard only
						//      covers the moment the request sits in that list, NOT the (much longer) time
						//      the harvester is actually in production. Every following scan therefore saw
						//      "nothing pending, still too few harvesters" and ordered another one, over
						//      and over, until a whole column of them finally rolled out at once.
						// Counting what is already IN PRODUCTION closes both: a replacement is requested
						// only while none is on its way.
						var pending = 0;
						foreach (var t in Info.HarvesterTypes)
							pending += unitBuilder.RequestedProductionCount(bot, t);
