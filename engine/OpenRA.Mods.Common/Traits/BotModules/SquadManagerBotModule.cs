...
namespace OpenRA.Mods.Common.Traits
{
	...
	public class SquadManagerBotModuleInfo : ConditionalTraitInfo
	{
		...
		[Desc("Hold up to this many ground units as a stationary reserve at the chokepoint supplied",
			"by an IBotChokepointProvider (attack-move, so they clear obstacles/trees on the way).",
			"These are kept out of attack squads and refilled from new units. 0 disables this.")]
		public readonly int ChokepointReserveSize = 0;

		[Desc("A reserve unit within this many cells of the chokepoint counts as in position.")]
		public readonly int ChokepointHoldRadius = 4;

		[Desc("Radius in cells around the chokepoint to scan for destructible trees and civilian buildings.",
			"Bridge actors, ore mines, oil derricks, and any type in ChokeClearExcludeTypes are excluded.")]
		public readonly int ChokeClearRadius = 5;

		[ActorReference]
		[Desc("Actor types to exclude from chokepoint obstacle clearing (ore mines, oil derricks, etc.).")]
		public readonly FrozenSet<string> ChokeClearExcludeTypes = FrozenSet<string>.Empty;
		...
		IBotEnabled, IBotTick, IBotRespondToAttack, IBotPositionsUpdated, IGameSaveTraitData, INotifyActorDisposing
	{
		...
		IBotChokepointProvider chokepointProvider;
		IBotBaseApproachProvider approachProvider;
		...
		protected override void Created(Actor self)
		{
			...
			chokepointProvider = self.Owner.PlayerActor.TraitsImplementing<IBotChokepointProvider>().FirstOrDefault();
		...
		void AssignRolesToIdleUnits(IBot bot)
		{
			...
			chokeReserve.RemoveWhere(unitCannotBeOrdered);
			secondaryReserve.RemoveWhere(unitCannotBeOrdered);
			...
			if (--assignRolesTicks <= 0)
			{
				...
				FindNewUnits(bot);
				if (Info.ChokepointReserveSize > 0)
					UpdateChokeReserve(bot);
			}
		...
		void UpdateChokeReserve(IBot bot)
		{
			if (chokepointProvider == null)
			{
				Log.Write("debug", "[AotChoke] chokepointProvider is null");
				return;
			}

			var choke = chokepointProvider.Chokepoint;
			if (!choke.HasValue)
			{
				Log.Write("debug", "[AotChoke] no chokepoint detected");
				return;
			}
			...
			// Secondary guard point (user spec): weaker units guard a SECONDARY approach — a BEACH landing
			// first (that is what secondaries are for), else another approach clearly distinct from the main
			// chokepoint. Without one, everyone holds the main chokepoint. NOBODY idles in the base — idle
			// units block building placement there.
			CPos? secondary = null;
			if (approachProvider != null)
			{
				var far = approachProvider.BaseApproaches
				...
				secondary = beaches.Count > 0 ? beaches[0].Gate
					: far.Count > 0 ? far[0].Gate
					: null;
			}

			Log.Write("debug", $"[AotChoke] choke={choke.Value} secondary={secondary?.ToString() ?? "none"} " +
				$"reserve={chokeReserve.Count}/{Info.ChokepointReserveSize} secReserve={secondaryReserve.Count} idle={unitsHangingAroundTheBase.Count}");

			// Assign ALL idle ground units: the STRONGER ones (vehicles/tanks) fill the main-choke reserve
			// first; everyone else guards the secondary approach (or the main choke when there is none).
			static bool IsStrong(Actor a) => a.Info.TraitInfos<TargetableInfo>().Any(t => t.TargetTypes.Contains("Vehicle") || t.TargetTypes.Contains("Tank"));
			foreach (var unit in unitsHangingAroundTheBase.OrderByDescending(IsStrong).ToList())
			{
				unitsHangingAroundTheBase.Remove(unit);
				if (chokeReserve.Count < Info.ChokepointReserveSize)
			...
			var holdR2 = Info.ChokepointHoldRadius * Info.ChokepointHoldRadius;

			// Move the secondary guards (fall back to the main choke when no secondary approach exists).
			var secTarget = secondary ?? choke.Value;
			var secOut = secondaryReserve.Where(a => !unitCannotBeOrdered(a) && (a.Location - secTarget).LengthSquared > holdR2).ToList();
			if (secOut.Count > 0)
				bot.QueueOrder(new Order("AttackMove", null, Target.FromCell(World, secTarget), false, groupedActors: secOut.ToArray()));

			var readyUnits = chokeReserve.Where(a => !unitCannotBeOrdered(a)).ToList();
			...
			// Step 1: move units that aren't at the chokepoint yet.
			if (outOfPosition.Count > 0)
				bot.QueueOrder(new Order("AttackMove", null, Target.FromCell(World, choke.Value), false, groupedActors: outOfPosition.ToArray()));

			// Step 2: units already in position scan the area for destructible neutral/civilian obstacles
			// (trees, civ buildings) and ForceAttack them — regular Attack ignores non-enemies.
			if (inPosition.Count == 0)
				return;

			var chokeW = World.Map.CenterOfCell(choke.Value);
			var obstacles = World.FindActorsInCircle(chokeW, WDist.FromCells(Info.ChokeClearRadius))
				.Where(a => !a.IsDead && a.IsInWorld
					&& a.Info.HasTraitInfo<HealthInfo>()
					&& a.Owner.NonCombatant
					&& Player.RelationshipWith(a.Owner) != PlayerRelationship.Ally
					&& !Info.ChokeClearExcludeTypes.Contains(a.Info.Name)
			...
			Log.Write("debug", $"[AotChoke] clearing {obstacles.Count} obstacle(s), nearest={obstacles[0].Info.Name}@{obstacles[0].Location}");

			// Distribute in-position units across the two nearest obstacles.
			var targets = obstacles.Take(2).ToList();
			for (var i = 0; i < inPosition.Count; i++)
				bot.QueueOrder(new Order("ForceAttack", inPosition[i], Target.FromActor(targets[i % targets.Count]), false));
		}
		...
		void CreateAttackForce(IBot bot)
		{
			...
			if (unitsHangingAroundTheBase.Count >= randomizedSquadSize)
			{
				...
				// chokeReserve units are already removed from unitsHangingAroundTheBase, so no filter needed.
