	...
	sealed class BaseBuilderQueueManager
	{
		...
		bool TickQueue(IBot bot, ProductionQueue queue)
		{
			...
			else if (currentBuilding != null && currentBuilding.Done)
			{
				...
				else
				{
					...
					else if (baseBuilder.Info.SilosNearOreMines > 0 && baseBuilder.Info.SiloTypes.Contains(actorInfo.Name))
		...
		ActorInfo GetProducibleBuilding(FrozenSet<string> actors, IEnumerable<ActorInfo> buildables, Func<ActorInfo, int> orderBy = null)
		{
			var available = buildables.Where(actor =>
			{
				...
				// Count finished AND in-production buildings, so this priority (e.g. the
					// "spend surplus cash on production" step) cannot overshoot the limit.
					var producing = baseBuilder.BuildingsBeingProduced.TryGetValue(actor.Name, out var bp) ? bp : 0;
					return playerBuildings.Count(a => a.Info.Name == actor.Name) + producing < limit;
			});
		...
		ActorInfo ChooseBuildingToBuild(ProductionQueue queue)
		{
			...
			// Early economy: ensure a silo near an ore mine (e.g. the delivery dock for ore transporters).
			// Prioritised right after power and before refineries, so the ore economy can start flowing.
			if (baseBuilder.Info.SilosNearOreMines > 0
				&& world.ActorsHavingTrait<OreMineDurability>().Any(a => !a.Disposed))
			{
				var siloCount = playerBuildings.Count(a => baseBuilder.Info.SiloTypes.Contains(a.Info.Name));
				foreach (var siloType in baseBuilder.Info.SiloTypes)
					if (baseBuilder.BuildingsBeingProduced.TryGetValue(siloType, out var beingBuilt))
						siloCount += beingBuilt;

				if (siloCount < baseBuilder.Info.SilosNearOreMines)
				{
					var silo = GetProducibleBuilding(baseBuilder.Info.SiloTypes, buildableThings);
					if (silo != null && HasSufficientPowerForActor(silo))
					{
						AIUtils.BotDebug("{0} decided to build {1}: Priority override (silo near ore mine)", queue.Actor.Owner, silo.Name);
						return silo;
					}
		...
		(CPos? Location, CPos? BaseCenter, int Variant) ChooseBuildLocation(string actorType, bool distanceToBaseIsImportant, BuildingType type)
		{
			...
			// Age of Tiberium masterplan: reserved bulk footprints (power/tech/prod/stealth blocks) are
			// off-limits to FindPos placements, so ordinary buildings only fill the gaps between blocks.
			// Bulk buildings never reach FindPos for their own block — TryGetBulkGridSlot routes them first.
			var aotLayout = player.PlayerActor.TraitOrDefault<AotBaseLayoutManager>();
			...
			(CPos? Location, CPos Center, int Variant) FindPos(CPos center, CPos target, int minRange, int maxRange)
			{
				...
				foreach (var cell in cells)
				{
					...
					// Keep out of the masterplan's reserved bulk blocks — only the gaps between them.
					if (aotLayout != null && vbi.Tiles(cell).Any(aotLayout.IsReservedForBulk))
						continue;
			...
			// Age of Tiberium base masterplan: cluster ordinary buildings into functional bulks
			// (power / production / tech / repair) around distinct anchors. Falls back to the
			// default placement below if the bulk area is not buildable yet.
			if (type == BuildingType.Building)
			{
				if (aotLayout != null)
				{
					// Power / production / tech bulks use a con-yard-avoiding, tiberium-free grid
					// so they pack into aligned, fenceable rows.
					if (aotLayout.TryGetBulkGridSlot(actorType, actorInfo, out var gridSlot))
						return (gridSlot, baseCenter, 0);

					// Other bulks (e.g. repair): pick the buildable cell CLOSEST to the bulk anchor.
					// (center != target is required for FindPos to sort by distance to the anchor.)
					if (aotLayout.TryGetBulkAnchor(actorType, out var bulkAnchor))
					{
						var bulkPos = FindPos(baseCenter, bulkAnchor, baseBuilder.Info.MinBaseRadius, baseBuilder.Info.MaxBaseRadius);
						if (bulkPos.Location != null)
							return bulkPos;
					}
				}
			}
			...
			switch (type)
			{
				...
				case BuildingType.Silo:
					...
					// Build the buildable cell closest to the nearest ore mine, so ore transporters
					// have the shortest possible delivery route (mine -> silo).
					var closestMine = world.ActorsHavingTrait<OreMineDurability>()
						.Where(a => !a.Disposed)
						.ClosestToIgnoringPath(world.Map.CenterOfCell(baseCenter));

					var mineCell = closestMine != null ? closestMine.Location : baseCenter;

					return FindPos(baseCenter, mineCell, baseBuilder.Info.MinBaseRadius, baseBuilder.Info.MaxBaseRadius);
