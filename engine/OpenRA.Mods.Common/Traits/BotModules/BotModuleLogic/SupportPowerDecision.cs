...
namespace OpenRA.Mods.Common.Traits
{
	...
	public class SupportPowerDecision
	{
		...
		public class Consideration
		{
			...
			// aotmod (User-Wunsch 2026-08-01): Superwaffen sollen gezielt bestimmte GEBAEUDE treffen
			// (Nuke/Ion -> Construction Yard, Tiberium Missile -> Kraftwerke, Mine Cluster ->
			// Verteidigungsanlagen, weil die typischerweise an Chokepoints stehen). Types allein
			// reicht dafuer nicht: das ist ein BitSet<TargetableType> mit groben Kategorien
			// (Structure/Vehicle/Infantry/...), es kann einzelne Aktoren nicht unterscheiden.
			// Leer = wie bisher, jeder Aktor der Types erfuellt zaehlt.
			[ActorReference]
			[Desc("Restrict this consideration to these actor types. Leave empty to consider any actor",
				"matching Types. Lets a decision aim at specific buildings (e.g. only power plants).")]
			public readonly FrozenSet<string> ActorTypes = FrozenSet<string>.Empty;
			...
			public int GetAttractiveness(Actor a, PlayerRelationship stance, Player firedBy)
			{
				...
				if (ActorTypes.Count > 0 && !ActorTypes.Contains(a.Info.Name))
					return 0;
			...
			public int GetAttractiveness(FrozenActor fa, PlayerRelationship stance)
			{
				...
				if (ActorTypes.Count > 0 && !ActorTypes.Contains(fa.Info.Name))
					return 0;
