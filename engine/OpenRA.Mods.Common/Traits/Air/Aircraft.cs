...
namespace OpenRA.Mods.Common.Traits
{
		...
		IActorPreviewInitModifier, IDeathActorInitModifier, IIssueDeployOrder, IIssueOrder, IResolveOrder, IOrderVoice
	{
		...
		{
			UnReserve();
			var (slot, res) = Reservable.TryReserve(target, self, this);
			if (slot != null)
			{
				reservation = res;
				reservedSlot = slot;
		...
		public WVec ReservedLandingOffset => reservedSlot?.Info.LandingOffset ?? WVec.Zero;
		public WAngle? ReservedLandingFacing => reservedSlot?.Info.LandingFacing;
		...
		public Activity MoveOntoTarget(Actor self, in Target target, in WVec offset, WAngle? facing, Color? targetLineColor, bool rotateLandingOffset)
		{
			return new Land(self, target, offset, facing, targetLineColor, rotateLandingOffset);
		}
		...
		public IEnumerable<IOrderTargeter> Orders
		{
			get
			{
				yield return new EnterAlliedActorTargeter<ReservableInfo>(
					"ForceEnter",
					6,
					Info.EnterCursor,
