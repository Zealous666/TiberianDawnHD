...
namespace OpenRA.Mods.Common.Traits
{
	...
	public class RepairableNear : IIssueOrder, IResolveOrder, IOrderVoice, IObservesVariables, INotifyCreated
	{
		...
		{
			return order.OrderString == "RepairNear" && (ShouldRepair() || ShouldRearm()) ? Info.Voice : null;
		}
		...
		void IResolveOrder.ResolveOrder(Actor self, Order order)
		{
			...
			if (!CanRepairAt(order.Target.Actor) || (!ShouldRepair() && !ShouldRearm()))
				return;
