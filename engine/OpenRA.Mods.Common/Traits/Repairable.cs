...
namespace OpenRA.Mods.Common.Traits
{
	...
	public class RepairableInfo : TraitInfo, Requires<IHealthInfo>, Requires<IMoveInfo>, IObservesVariablesInfo
	{
		...
		// aotmod (User-Fund 2026-08-01: "wenn eine Einheit nicht repariert werden kann, faehrt sie
		// auch mit ALT force-move nicht darauf"). Vanilla laesst den Repair-Befehl nur zu, wenn es
		// wirklich etwas zu reparieren oder aufzumunitionieren gibt -- eine unbeschaedigte Einheit
		// kommt gar nicht erst aufs Depot. Fuer den Verkauf am Depot (RA1/TS-Verhalten, siehe
		// Sellable auf ^Vehicle) muss sie aber genau das koennen. Ist nichts zu tun, parkt sie
		// dort (stayOnResupplier) statt sofort wieder abzufahren.
		[Desc("Allow being sent to a repair actor even with nothing to repair or rearm.",
			"The unit then simply parks there until ordered elsewhere.")]
		public readonly bool AllowUndamagedDocking = false;
	...
	public class Repairable : IIssueOrder, IResolveOrder, IOrderVoice, INotifyCreated, IObservesVariables
	{
			...
			get
			{
				if (!isAircraft)
					yield return new EnterAlliedActorTargeter<BuildingInfo>(
						...
						CanRepairAt,
						_ => CanRepair() || CanRearm() || Info.AllowUndamagedDocking);
			}
		}
		...
		void IResolveOrder.ResolveOrder(Actor self, Order order)
		{
			...
			if (!CanRepairAt(order.Target.Actor))
				return;

			var hasWork = CanRepair() || CanRearm();
			if (!hasWork && !Info.AllowUndamagedDocking)
				return;

			// Nichts zu tun -> trotzdem hinfahren (dockWhenIdle) und dort stehen bleiben
			// (stayOnResupplier), statt gar nicht loszufahren bzw. sofort wieder abzudrehen.
			self.QueueActivity(order.Queued, new Resupply(self, order.Target.Actor, new WDist(512), !hasWork, !hasWork));
