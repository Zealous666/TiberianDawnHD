...
namespace OpenRA.Mods.Common.Traits
{
		...
		INotifyCreated, INotifyKilled, ITransformActorInitModifier, INotifyTransform, ITick
	{
		...
		{
			// Skips buildings (no IFacing) — only vehicle carriers update passenger facing each tick
			// so passengers visually rotate with the transport (e.g. hover LST).
			if (facing.Value == null || cargo.Count == 0)
				return;

			foreach (var p in cargo)
				SetPassengerFacing(p);
		}
