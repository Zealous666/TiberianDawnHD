...
namespace OpenRA.Mods.Common.Traits
{
	...
	public class RepairsBridges : IIssueOrder, IResolveOrder, IOrderVoice
	{
		...
		public string VoicePhraseForOrder(Actor self, Order order)
		{
			...
			if (legacyHut != null)
				return legacyHut.BridgeDamageState == DamageState.Undamaged || legacyHut.Repairing || legacyHut.BridgeIsDangling ? null : info.Voice;
		...
		public void ResolveOrder(Actor self, Order order)
		{
			...
			if (order.OrderString == "RepairBridge" && order.Target.Type == TargetType.Actor)
			{
				...
				if (legacyHut != null)
				{
					if (legacyHut.BridgeDamageState == DamageState.Undamaged || legacyHut.Repairing || legacyHut.BridgeIsDangling)
						return;
				}
				else if (hut != null)
				{
		...
		sealed class RepairBridgeOrderTargeter : UnitOrderTargeter
		{
			...
			public override bool CanTargetActor(Actor self, Actor target, TargetModifiers modifiers, ref string cursor)
			{
				...
				if (legacyHut != null)
				{
					...
					if (damage == DamageState.Undamaged || legacyHut.Repairing || legacyHut.BridgeIsDangling)
