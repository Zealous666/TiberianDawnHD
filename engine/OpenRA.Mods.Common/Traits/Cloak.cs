...
namespace OpenRA.Mods.Common.Traits
{
	...
	public enum UncloakType
	{
		...
		// aotmod (User-Wunsch 2026-08-01, "Stealth Harvester"): Enttarnen waehrend des ERNTENS.
		// Dock deckt nur das Abladen an der Raffinerie ab -- das Aufsammeln auf dem Feld ist
		// kein Dock-Vorgang und hatte bisher gar keinen Uncloak-Trigger.
		Harvest = 2048,
	}
	...
	public class CloakInfo : PausableConditionalTraitInfo
	{
		...
		[Desc(
			...
			"'Harvest' is triggered each time a harvester collects a resource cell.",
		...
		INotifyHarvestAction, ISync
	{
		...
		// aotmod: Ernte-Trigger. Harvested() feuert pro eingesammelter Zelle, das haelt den
		// Harvester waehrend des gesamten Erntevorgangs sichtbar; danach greift der normale
		// CloakDelay wieder.
		void INotifyHarvestAction.Harvested(Actor self, string resourceType)
		{
			if (Info.UncloakOn.HasFlag(UncloakType.Harvest))
				Uncloak();
		}
