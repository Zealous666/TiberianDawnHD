...
namespace OpenRA.Mods.Common.Traits
{
	...
	public class PowerManager : INotifyCreated, ITick, ISync, IResolveOrder
	{
		...
		void UpdatePowerRequiringActors()
		{
			...
			// aotmod: der Spieler-Aktor selbst wird nie ueber World.Add() eingehaengt (Player.cs
			// baut ihn direkt mit "new Actor(...)"), taucht in ActorsWithTrait also nicht auf.
			// Traits auf dem Spieler-Aktor bekamen den Stromwechsel deshalb nie mit -- ein
			// GrantConditionOnPowerState dort blieb fuer immer auf seinem Startwert stehen.
			// Age of Tiberium braucht das fuer die Age-Powers, die am Spieler haengen und bei
			// Strommangel pausieren sollen wie jede andere Super Power.
			foreach (var t in self.TraitsImplementing<INotifyPowerLevelChanged>())
				t.PowerLevelChanged(self);
		}
