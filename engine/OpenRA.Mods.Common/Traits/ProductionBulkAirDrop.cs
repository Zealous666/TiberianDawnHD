...
namespace OpenRA.Mods.Common.Traits
{
	...
	public class ProductionBulkAirdropInfo : ProductionInfo
	{
		...
		[Desc("Direction the aircraft faces when touching down. Used to determine which map edge it approaches from (opposite side). Default WAngle.Zero = closest edge.")]
	...
	sealed class ProductionBulkAirdrop : Production
	{
		...
		public bool DeliverOrder(Actor producer, List<(ActorInfo Actor, int Resources, int Cash)> orderedActors, string productionType, BulkProductionQueue queue)
		{
			...
			var startPos = info.Facing != WAngle.Zero
				? ChooseEdgeCellForLanding(producer.World.Map, producer.Location, info.Facing)
				: producer.World.Map.ChooseClosestEdgeCell(producer.Location);
		...
		// When Facing is configured, spawn the transport from the edge the aircraft approaches from
		// (the side OPPOSITE the landing facing). Facing 768 = land West = approach from East edge.
		static CPos ChooseEdgeCellForLanding(Map map, CPos loc, WAngle landingFacing)
		{
			// Approach direction = 180° from landing facing
			var approachAngle = (landingFacing.Angle + 512) % 1024;
			var bounds = map.Bounds;

			// Place a biased point far outside the map on the correct edge, then clamp to boundary
			CPos biasedLoc;
			if (approachAngle >= 128 && approachAngle < 384) // East
				biasedLoc = new CPos(bounds.Right + bounds.Width, loc.Y);
			else if (approachAngle >= 384 && approachAngle < 640) // South
				biasedLoc = new CPos(loc.X, bounds.Bottom + bounds.Height);
			else if (approachAngle >= 640 && approachAngle < 896) // West
				biasedLoc = new CPos(bounds.Left - bounds.Width, loc.Y);
			else // North
				biasedLoc = new CPos(loc.X, bounds.Top - bounds.Height);

			return map.ChooseClosestEdgeCell(map.Clamp(biasedLoc));
		}
	}
}
