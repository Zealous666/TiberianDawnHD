...
namespace OpenRA.Mods.Common.Traits
{
	public class BuildableInfo : TraitInfo<Buildable>
	{
		...
		[Desc("If false, this actor will not appear as a locked placeholder in the production palette " +
			"when the queue's ShowLockedItems is enabled. Use for superpowers or other items " +
			"that should only appear once their prerequisites are fully met.")]
		public readonly bool ShowWhenLocked = true;

		[Desc("Actors sharing this group occupy a single palette slot when the queue's ShowLockedItems " +
			"is enabled: visible members show their real icons, a fully hidden group shows one " +
			"locked placeholder. Defaults to the actor name (each actor is its own group).")]
		public readonly string LockedGroup = null;
