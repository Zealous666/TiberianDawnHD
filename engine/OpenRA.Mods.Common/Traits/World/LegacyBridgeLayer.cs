...
namespace OpenRA.Mods.Common.Traits
{
	...
	public class LegacyBridgeLayer : IWorldLoaded
	{
		...
		public void WorldLoaded(World w, WorldRenderer wr)
		{
			...
			// Take all templates to overlay from the map. A bridge actor registers its template IDs
			// globally, but a raw template ID can be reused for completely different art in another
			// tileset (e.g. AOT_TEMPERAT reuses the cnc bridge IDs 165-172 for cliff pieces, and the
			// standard tilesets reuse the AOT bridge IDs 131-134 for road pieces). Only overlay a cell
			// when the tileset's own template for that ID is actually a bridge (Categories: Bridge) —
			// otherwise cliffs/roads sharing an ID would wrongly become destructible bridges.
			foreach (var cell in w.Map.AllCells.Where(cell =>
			{
				var type = w.Map.Tiles[cell].Type;
				return bridgeTypes.ContainsKey(type)
					&& terrainInfo.Templates.TryGetValue(type, out var template)
					&& template.Categories.Contains("Bridge");
			}))
