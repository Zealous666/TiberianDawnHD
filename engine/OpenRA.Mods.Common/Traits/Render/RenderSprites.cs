...
namespace OpenRA.Mods.Common.Traits.Render
{
	...
	// aotmod: Map-Editor-Farbwaehler pro platziertem Aktor (Gebaeude/Einheit) - ueberschreibt die
	// spaetere Spieler-/PlayerReference-Farbe rein visuell. Siehe [[editor-actor-color-override]].
	public class AotEditorColorOverrideInit(string value) : ValueActorInit<string>(value), ISingleInstanceInit
	{
	}

	[Desc("Render trait fundament that won't work without additional With* render traits.")]
	public class RenderSpritesInfo : TraitInfo, IRenderActorPreviewInfo, IEditorActorOptions
	{
		// aotmod: feste Palette an klassischen TD/RA-Spielerfarben fuer den Editor-Farbwaehler.
		// Reihenfolge = Anzeige-Reihenfolge im Dropdown.
		public static readonly (string Key, string Label, Color Color)[] AotEditorColors =
		[
			("gold", "Gold", Color.FromArgb(0xFFFCC200)),
			("red", "Red", Color.FromArgb(0xFFE00000)),
			("lightblue", "Light Blue", Color.FromArgb(0xFF3080FF)),
			("orange", "Orange", Color.FromArgb(0xFFFF8000)),
			("green", "Green", Color.FromArgb(0xFF00A000)),
			("teal", "Teal", Color.FromArgb(0xFF00A0A0)),
			("salmon", "Salmon", Color.FromArgb(0xFFFF8080)),
			("grey", "Grey", Color.FromArgb(0xFFC0C0C0)),
		];

		IEnumerable<EditorActorOption> IEditorActorOptions.ActorOptions(ActorInfo ai, World world)
		{
			yield return new EditorActorDropdown("Player Color Override", 1,
				actor =>
				{
					var labels = new Dictionary<string, string> { { "", "(Default)" } };
					foreach (var c in AotEditorColors)
						labels.Add(c.Key, c.Label);
					return labels;
				},
				(actor, labels) =>
				{
					var init = actor.GetInitOrDefault<AotEditorColorOverrideInit>();
					return init?.Value ?? "";
				},
				(actor, value) =>
				{
					actor.RemoveInit<AotEditorColorOverrideInit>();
					if (!string.IsNullOrEmpty(value))
						actor.ReplaceInit(new AotEditorColorOverrideInit(value));
				});
		}
	...
	public class RenderSprites : IRender, ITick, INotifyOwnerChanged, INotifyEffectiveOwnerChanged, IActorPreviewInitModifier
	{
		...
		sealed class AnimationWrapper
		{
			...
			public void CachePalette(WorldRenderer wr, Player owner, (string Key, Color Color)? colorOverride)
			{
				if (IsPlayerPalette && colorOverride != null)
				{
					// aotmod: Editor-Farbwaehler - registriert (falls noch nicht geschehen) eine feste,
					// von der echten Spielerfarbe unabhaengige Palette und nutzt diese statt owner.InternalName.
					wr.UpdatePalettesForPlayer(colorOverride.Value.Key, colorOverride.Value.Color, true);
					PaletteReference = wr.Palette(Palette + colorOverride.Value.Key);
				}
				else
		...
		// aotmod: Editor-Farbwaehler - falls am Aktor gesetzt, ueberschreibt dies die aus der
		// echten PlayerReference/Lobby-Farbe abgeleitete Palette rein visuell.
		readonly (string Key, Color Color)? colorOverride;
		...
		public RenderSprites(ActorInitializer init, RenderSpritesInfo info)
		{
			...
			var overrideKey = init.GetOrDefault<AotEditorColorOverrideInit>()?.Value;
			if (!string.IsNullOrEmpty(overrideKey))
			{
				foreach (var c in RenderSpritesInfo.AotEditorColors)
				{
					if (c.Key == overrideKey)
					{
						colorOverride = ("aot-editorcolor-" + c.Key, c.Color);
						break;
					}
				}
			}
			...
			if (shouldRefreshPalettes)
			{
				...
				foreach (var a in anims)
				{
					if (a.PaletteReference == null)
					{
						...
						a.CachePalette(wr, owner, colorOverride);
					}
				}
			}
