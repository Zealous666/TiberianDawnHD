...
namespace OpenRA.Mods.Common.Widgets
{
	...
	public class ProductionTabsWidget : Widget
	{
		...
		// aotmod: erkennt Selektions-Aenderungen, um bei ATEC/STEC automatisch das Upgrade-Menue zu oeffnen.
		int selectionHash = -1;
		...
		public override void Tick()
		{
			...
			// aotmod: Beim Selektieren eines Aktors mit AotOpensProductionTabOnSelection (z.B. ATEC/STEC)
			// automatisch zur gewuenschten Tab-Gruppe (z.B. Upgrade) wechseln.
			if (world.Selection.Hash != selectionHash)
			{
				...
				if (world.LocalPlayer != null)
				{
					...
					{
						if (a.Owner != world.LocalPlayer || a.IsDead || !a.IsInWorld)
							continue;

						var opener = a.Info.TraitInfoOrDefault<AotOpensProductionTabOnSelectionInfo>();
						if (opener == null)
							continue;

						if (opener.Group != null && queueGroup != opener.Group
							&& Groups.TryGetValue(opener.Group, out var group) && group.Tabs.Count > 0)
							QueueGroup = opener.Group;
