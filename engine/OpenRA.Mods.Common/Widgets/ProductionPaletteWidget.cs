...
namespace OpenRA.Mods.Common.Widgets
{
	...
	public class ProductionPaletteWidget : Widget
	{
		...
		[Desc("ImageCollection name for the placeholder icon shown on items that are not yet visible.")]
		public readonly string LockedIconCollection = null;

		[Desc("Region name within the LockedIconCollection for the placeholder sprite.")]
		public readonly string LockedIconRegion = "locked";

		[Desc("Text overlay drawn over items whose BuildLimit is reached, on queues with ShowUpgradedOverlay.")]
		public readonly string UpgradedText = "Upgraded";

		[Desc("Text overlay drawn over items whose BuildLimit is reached, on all other queues. \\n breaks lines.")]
		public readonly string MaximumBuiltText = "Maximum\\nBuilt";

		public readonly Color UpgradedTextColor = Color.FromArgb(255, 200, 200, 200);
		...
		Sprite lockedSprite;
		PaletteReference lockedPalette;

		readonly WorldRenderer worldRenderer;

		SpriteFont overlayFont, symbolFont;
		float2 iconOffset, holdOffset, readyOffset, timeOffset, infiniteOffset, upgradedOffset;
		string[] maximumBuiltLines;
		float2[] maximumBuiltOffsets;
		...
		public ProductionPaletteWidget(ModData modData, OrderManager orderManager, World world, WorldRenderer worldRenderer)
		{
			...
			GetTooltipIcon = () => TooltipIcon?.IsLocked == true ? null : TooltipIcon;
		...
		public override void Initialize(WidgetArgs args)
		{
			...
			if (LockedIconCollection != null)
			{
				lockedSprite = ChromeProvider.GetImage(LockedIconCollection, LockedIconRegion);
				lockedPalette = worldRenderer.Palette("chrome");
			}
			...
			upgradedOffset = iconOffset - overlayFont.Measure(UpgradedText) / 2;

			// "\n" in MaximumBuiltText breaks lines; each line is centered individually.
			maximumBuiltLines = MaximumBuiltText.Split("\\n", StringSplitOptions.None);
			maximumBuiltOffsets = new float2[maximumBuiltLines.Length];
			var totalHeight = maximumBuiltLines.Sum(l => overlayFont.Measure(l).Y);
			var lineY = iconOffset.Y - totalHeight / 2f;
			for (var i = 0; i < maximumBuiltLines.Length; i++)
			{
				var lineSize = overlayFont.Measure(maximumBuiltLines[i]);
				maximumBuiltOffsets[i] = new float2(iconOffset.X - lineSize.X / 2f, lineY);
				lineY += lineSize.Y;
			}
		...
		public void RefreshIcons()
		{
			...
			// Prefer a faction-specific placeholder collection ("<collection>-<faction>") when defined.
			var factionLockedSprite = lockedSprite;
			if (lockedSprite != null && !string.IsNullOrEmpty(faction))
			{
				var factionSprite = ChromeProvider.TryGetImage(LockedIconCollection + "-" + faction, LockedIconRegion);
				if (factionSprite != null)
					factionLockedSprite = factionSprite;
			}
			...
			foreach (var item in AllBuildables.Skip(IconRowOffset * Columns).Take(MaxIconRowOffset * Columns))
			{
				...
				var isLocked = lockedSprite != null && currentQueue.IsLocked(item);
				...
				var pi = new ProductionIcon()
				{
					...
					Sprite = isLocked ? factionLockedSprite : icon.Image,
					Palette = isLocked ? lockedPalette : worldRenderer.Palette(palette),
		...
		public override void Draw()
		{
			...
			foreach (var icon in icons.Values)
			{
				...
				else if (!icon.IsLocked && !buildableItems.Any(a => a.Name == icon.Name))
			...
			// Overlays
			var atLimitItems = CurrentQueue.ItemsAtBuildLimit();
			foreach (var icon in icons.Values)
			{
				...
				else if (!icon.IsLocked && atLimitItems.Contains(icon.Name))
				{
					if (CurrentQueue.Info.ShowUpgradedOverlay)
						overlayFont.DrawTextWithContrast(UpgradedText, icon.Pos + upgradedOffset, UpgradedTextColor, Color.Black, 1);
					else
						for (var i = 0; i < maximumBuiltLines.Length; i++)
							overlayFont.DrawTextWithContrast(maximumBuiltLines[i], icon.Pos + maximumBuiltOffsets[i], UpgradedTextColor, Color.Black, 1);
				}
