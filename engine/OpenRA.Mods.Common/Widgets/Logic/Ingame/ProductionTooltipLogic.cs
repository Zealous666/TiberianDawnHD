	...
	public class ProductionTooltipLogic : ChromeLogic
	{
		...
		public ProductionTooltipLogic(Widget widget, TooltipContainerWidget tooltipContainer, Player player, Func<ProductionIcon> getTooltipIcon)
		{
			...
			tooltipContainer.BeforeRender = () =>
			{
				...
				// aotmod 2026-08-05: fehlende Voraussetzungen rot hervorheben (User-Wunsch). Ein
				// LabelWidget kann keine Teile einer Zeile einfaerben -- LabelWithHighlightWidget
				// schon: Text zwischen '<' und '>' wird in HighlightColor gezeichnet. Die noch
				// nicht erfuellten Namen werden daher eingeklammert, die erfuellten bleiben normal.
				...
				var prereqs = buildable.Prerequisites
					...
					.Select(a =>
					{
						...
						return techTree != null && !techTree.HasPrerequisites([a]) ? $"<{name}>" : name;
					})
					.ToList();
