...
namespace OpenRA.Mods.Common.Widgets
{
	public class SupportPowersWidget : Widget
	{
		...
		[Desc("Ticks for one full pulse of an icon that reports FlashIcon.")]
		public readonly int FlashPeriod = 50;

		[Desc("Peak alpha of that pulse. Deliberately gentle -- a signal, not a strobe.")]
		public readonly int FlashMaxAlpha = 70;
		...
		public override void Draw()
		{
			...
			foreach (var p in icons.Values)
			{
				...
				// Attention pulse for a power that can be bought RIGHT NOW (Age upgrades ask for this;
				// every other power leaves FlashIcon false and looks exactly as before). A soft white
				// wash over the icon rather than blinking it away, so the artwork stays readable.
				if (sp.FlashIcon)
				{
					var phase = Game.LocalTick % FlashPeriod;
					var alpha = (int)(FlashMaxAlpha * (1 - Math.Abs(2f * phase / FlashPeriod - 1)));
					if (alpha > 0)
						WidgetUtils.FillRectWithColor(
							new Rectangle((int)p.Pos.X, (int)p.Pos.Y, IconSize.X, IconSize.Y),
