...
namespace OpenRA.Mods.Common.Effects
{
	public class NukeLaunch : IProjectile, ISpatiallyPartitionable
	{
		...
		// aotmod: Firestorm Defense Matrix Interception
		bool aotIntercepted;
		...
		public void Tick(World world)
		{
			...
			// 1) Direkt: Flugsaeule (XY) liegt auf einer aktiven Matrix-Zelle -> sofort abfangen.
			// 2) Einschluss: Zielzelle liegt in einem von Matrix-Zellen abgeriegelten Gebiet ->
			//    beim Sinkflug unterhalb der Abfanghoehe zerstoeren (NukeLaunch fliegt nicht
			//    horizontal, daher ist das der einzig sinnvolle "Durchflug"-Ersatz).
				...
				if (isDescending)
				{
					if (!aotEnclosureChecked)
					{
						...
						aotEnclosingMatrix = AotFirestormMatrixPower.FindEnclosingInterceptor(
							world, world.Map.CellContaining(descendTarget), firedBy);
					}
		...
		{
			// Kein Warhead-Impact — die Nuke wird ersatzlos zerstoert (nur Effekt + Sound)
			aotIntercepted = true;
			detonated = true;
