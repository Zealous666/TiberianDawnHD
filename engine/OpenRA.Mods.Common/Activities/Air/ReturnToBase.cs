	...
	public class ReturnToBase : Activity
	{
		...
		public override bool Tick(Actor self)
		{
			...
			if (ShouldLandAtBuilding(self, dest))
			{
				...
				// If the chosen resupplier is full, immediately try the next available one.
				if (aircraft.ReservedActor == null)
				{
					dest = ChooseResupplier(self, true);
					if (dest != null)
						aircraft.MakeReservation(dest);
				...
				// If still no slot (all resuppliers full), hover near the nearest and retry later.
				if (aircraft.ReservedActor == null)
				{
					dest = null;
					QueueChild(new FlyIdle(self, aircraft.Info.NumberOfTicksToVerifyAvailableAirport));
				...
				// Slot may have moved to a different resupplier — sync dest with the actual reservation.
				dest = aircraft.ReservedActor;

				// Use the reserved slot's landing offset if specified, otherwise fall back to nearest Exit.
				var offset = aircraft.ReservedLandingOffset;
				facing = aircraft.ReservedLandingFacing;
				if (offset == WVec.Zero)
				{
				...
				var rotateLandingOffset = offset != WVec.Zero && dest.TraitOrDefault<Mobile>() != null;
				QueueChild(aircraft.MoveOntoTarget(self, Target.FromActor(dest), offset, facing, Color.Green, rotateLandingOffset));
