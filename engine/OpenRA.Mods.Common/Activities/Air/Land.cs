...
namespace OpenRA.Mods.Common.Activities
{
	public class Land : Activity
	{
		...
		readonly bool rotateLandingOffset;
		...
		public Land(Actor self, in Target target, WDist landRange, in WVec offset, WAngle? facing = null, CPos[] clearCells = null, Color? targetLineColor = null, bool rotateLandingOffset = false)
		{
			...
			this.target = target;
			this.offset = offset;
			this.rotateLandingOffset = rotateLandingOffset;
		...
		public override bool Tick(Actor self)
		{
			...
			// For mobile targets (e.g. carrier), rotate the slot offset by the target's current facing
			// so the landing spot tracks the bow/stern even as the carrier turns.
			WVec effectiveOffset;
			if (rotateLandingOffset && offset != WVec.Zero && target.Type == TargetType.Actor)
			{
				var targetFacing = target.Actor.TraitOrDefault<IFacing>();
				effectiveOffset = targetFacing != null
					? offset.Rotate(WRot.FromYaw(targetFacing.Facing))
					: offset;
			}
			else
				effectiveOffset = offset;

			targetPosition = target.CenterPosition + effectiveOffset;
			...
			if (!landingInitiated)
			{
				...
				// Guard: an aircraft doesn't block itself in CanLand, so influence must be
				// removed before re-adding to avoid the double-AddInfluence exception.
