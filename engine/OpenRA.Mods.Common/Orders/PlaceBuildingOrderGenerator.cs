		...
		sealed class VariantWrapper
		{
			...
			public VariantWrapper(WorldRenderer wr, ProductionQueue queue, ActorInfo ai)
			{
				...
				if (previewGeneratorInfo != null)
				{
					...
					// aotmod: make the placement ghost reflect the placing player's TechTree state for
					// condition-gated single-actor sprite switches (e.g. Age-2 re-skins). See
					// AotForwardTechTreeConditionToPlacementPreview.cs for why this can't be done via
					// the generic IActorPreviewInitInfo hook above (no Owner/Player in scope there).
					var techForward = ActorInfo.TraitInfoOrDefault<AotForwardTechTreeConditionToPlacementPreviewInfo>();
					if (techForward != null && techForward.Condition != null && !td.Contains<AotPreviewConditionsInit>())
					{
						var techTree = queue.Actor.Owner.PlayerActor.TraitOrDefault<TechTree>();
						var granted = techTree != null && techTree.HasPrerequisites(techForward.Prerequisites);
						td.Add(new AotPreviewConditionsInit(new Dictionary<string, int> { { techForward.Condition, granted ? 1 : 0 } }));
					}
